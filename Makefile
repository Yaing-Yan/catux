...
OBJS := $(BUILD)/boot.o $(BUILD)/isr.o $(BUILD)/console.o $(BUILD)/gdt.o \
        $(BUILD)/idt.o $(BUILD)/pic.o $(BUILD)/timer.o $(BUILD)/keyboard.o \
        $(BUILD)/mem.o $(BUILD)/paging.o $(BUILD)/heap.o $(BUILD)/fs.o \
        $(BUILD)/syscall.o $(BUILD)/task.o $(BUILD)/string.o $(BUILD)/shell.o \
        $(BUILD)/main.o
...
# 用户态程序：独立编译、链接到 0x40000000，objcopy 成扁平二进制
USER_CFLAGS := -target i386-elf -ffreestanding -fno-pic -fno-pie \
               -fno-stack-protector -fno-builtin -mno-mmx -mno-sse \
               -Wall -Wextra -O2

$(BUILD)/hello.elf: user/hello.c user/link.ld | $(BUILD)
	$(CC) $(USER_CFLAGS) -c user/hello.c -o $(BUILD)/hello.o
	$(LD) -m elf_i386 -T user/link.ld -nostdlib -o $@ $(BUILD)/hello.o
...
$(INITRD): $(INITRD_SRC) $(BUILD)/hello.bin
	tar -cf $@ -C initrd . -C $(CURDIR)/$(BUILD) hello.bin
